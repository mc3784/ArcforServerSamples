...
while [ $retryCount -gt 0 ]; do
  ...
  if [ $? != 0 ]; then wget -qO- --method=PUT --body-data="{\"subscriptionId\":\"$subscriptionID\",\"resourceGroup\":\"$RESOURCE_GROUP\",\"tenantId\":\"$TENANT_ID\",\"location\":\"$LOCATION\",\"correlationId\":\"$CORRELATION_ID\",\"authType\":\"$AUTH_TYPE\",\"operation\":\"onboarding\",\"messageType\":\"DownloadScriptFailed\",\"message\":\"$output\"}" "https://gbl.his.arc.azure.com/log" &> /dev/null || true; fi;
  echo "$output";
  if [ $? -eq 0 ]; then
    break
  else
